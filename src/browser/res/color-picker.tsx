...
export class ColorPicker {
    ...
    private extractPalette(data: ImageData) {
        ...
        for (let i = 0; i < colors.length; i += 4) {
            ...
            if (count === undefined) palette.set(color, 1);
            else palette.set(color, count + 1);
        }

        const sorted = Array.from(palette).sort((a, b) => b[1] - a[1]);
        const p = sorted.slice(0, Math.min(sorted.length, 5));
        if (p.length > 0) {
            return p.map(([color, freq]) => [(color >> 16) & 0xff, (color >> 8) & 0xff, color & 0xff, 255]);
        }
        return [];
    }
    ...
    private quantization_(rgbValues: RGB[], depth: number): RGB[] {
        ...
        rgbValues.sort((p1, p2) =>
            // @ts-ignore
            p1[componentToSortBy] - p2[componentToSortBy]);
